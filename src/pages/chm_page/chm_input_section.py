...
from modules.utils.chm_utils import (getParameterAndUnitTypes,
                                     getParameterTypeNum, parameterItem)
from modules.utils.logic_utils import is_real_number
from modules.widgets.dialogs import (deleteBox, duplicateSampleOverrideDialog,
                                     saveMessageDialog, showErrorDialog)
...
def save_error_handling(self, new_data, item):
    ...
    if(old_jobNum != new_jobNum or old_sampleNum != new_sampleNum):
        ...
        if(existing_data_check):
            response = saveMessageDialog(self, 'Overwrite Data?', f'Are you sure you want overwrite existing data for {new_jobName} and delete data for {old_jobName} ')
            ...
            response = saveMessageDialog(self, 'Overwrite Data?', f'Are you sure you want save {new_jobName} and delete {old_jobNum} ?')
        ...
        response = saveMessageDialog(self, 'Overwrite Data?', f'Are you sure you want overwrite existing data for {new_jobName}?')
